...
export default function Root() {
	...
	useEffect(() => {
		const getUser = async () => {
			let { data: user_data, error: user_error } = await supabase.from("user").select().eq("id", internalUserId);
	...
	useEffect(() => {
		...
		const getUserGroupCount = async () => {
			let { count } = await supabase.from("group_members").select("*", { count: "exact", head: true }).eq("user_id", user.id);
				...
				<nav>
					...
					<div className="user-info">
						<div className="avatar-field">
							<Link to="/profile">
								<img id="avatar" src={user.avatar ? user.avatar : samantha} alt={`${user.first_name}'s avatar`} />
							</Link>
							<img
              id="company-logo"
              src={userCompany && userCompany.logo ? userCompany.logo : companyLogo}
