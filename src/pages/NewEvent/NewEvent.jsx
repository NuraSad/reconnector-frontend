...
export default function NewEvent() {
	...
	const notCreate = () => toast.warn("Event name AND description are Required!");
	...
	async function handleSubmit(event) {
		...
		if (imageFile) {
			...
			const { data: uploadData, error: uploadError } = await supabase.storage.from("eventImages").upload(auth_user_id + "/" + uuidv4(), imageFile);
          ...
          : "Create New Event"}
			...
			<div className="newEvent__col-1">
				<div className="newEvent__input">
					<label></label>
					<SelectInput
						dropDownInfo={Object.keys(groups).map((key) => ({ id: key, name: groups[key] }))}
						labelName={"What group is this event under?"}
						name="groupList"
						onChangeFunction={(name, value) => onChange(name, value, "groupList", setNewEvent)}
					/>
				</div>
				<div className="newEvent__input">
